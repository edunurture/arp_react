...
const KeyIndicatorSetup = () => {
  ...
  // Mock table data (matches HTML sample; hook API later)
  ...
  const onTableView = () => {
    ...
    if (!r) return
    // Minimal "view" mapping into form (HTML has view)
    setForm((p) => ({
      ...p,
  ...
  const onTableEdit = () => {
    // HTML shows Edit button in top toolbar; map to enable editing for current row
    const r = getSelectedRow()
    if (!r) return
    setIsEdit(true)
    setForm((p) => ({
  ...
  const onSave = (e) => {
    ...
    // Demo: update the criteria row aggregates (kis etc. are not derived here)
    const id = String(form.criteriaNo).trim()
    const nextRow = {
      id,
      criteriaNo: id,
      name: rows.find((r) => r.id === id)?.name ?? 'Criteria',
      kis: rows.find((r) => r.id === id)?.kis ?? '0',
      qlms: rows.find((r) => r.id === id)?.qlms ?? '0',
      qnms: rows.find((r) => r.id === id)?.qnms ?? '0',
    ...
    <CRow>
      <CCol xs={12}>
          ...
          <CCardBody>
            <CForm onSubmit={onSave}>
              <CRow className="g-3">
                ...
                <CCol md={3}>
                  <CFormLabel>Key Indicator (KI)</CFormLabel>
                </CCol>
                <CCol md={3}>
                  <CFormInput
                    ...
                    disabled={!isEdit}
                    required
                    placeholder="Enter Key Indicator"
                  />
                </CCol>

                {/* Row 3 */}
                <CCol md={3}>
                  <CFormLabel>KI Description</CFormLabel>
                </CCol>
                <CCol md={3}>
                  <CFormInput
                    value={form.kiDescription}
                    onChange={onChange('kiDescription')}
                    disabled={!isEdit}
                    required
                    placeholder="Enter KI Description"
                  />
                </CCol>
        ...
        <CCard className="mb-3" ref={tableRef}>
          <CCardHeader className="d-flex justify-content-between align-items-center">
            <strong>CRITERIA AND KEY INDICATORS DETAILS</strong>
