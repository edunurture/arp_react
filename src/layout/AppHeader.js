...
const AppHeader = () => {
  ...
  const handleHamburgerClick = () => {
    if (isMobile()) {
      ...
      // Desktop: docking (icons-only) toggle
      dispatch({ type: 'set', sidebarUnfoldable: !sidebarUnfoldable })
      dispatch({ type: 'set', sidebarShow: true }) // ensure not hidden
    }
  }
  ...
  return (
    <CHeader position="sticky" className="mb-4 p-0" ref={headerRef}>
      <CContainer className="border-bottom px-4" fluid>
        <CHeaderToggler onClick={handleHamburgerClick} style={{ marginInlineStart: '-14px' }}>
